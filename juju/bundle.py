...
class BundleHandler:
    ...
    def __init__(self, model, trusted=False, forced=False):
        ...
        self.bundle_facade = client.BundleFacade.from_connection(model.connection())
        self.client_facade = client.ClientFacade.from_connection(model.connection())
        self.app_facade = client.ApplicationFacade.from_connection(model.connection())
        self.ann_facade = client.AnnotationsFacade.from_connection(model.connection())
        ...
        best_facade_version = client.CharmsFacade.best_facade_version(
            model.connection()
        )
        if best_facade_version is not None and best_facade_version > 2:
            self.charms_facade = client.CharmsFacade.from_connection(model.connection())
        else:
        ...
        for app_name in self.applications:
            ...
            try:
                ...
                if not (
                    charm_path.is_dir()
                    or (
                        charm_path.is_file() and charm_path.suffix in (".charm", ".zip")
                    )
                ):
                    continue
        ...
        if apps:
            ...
            for app_name, charm_url, (charm_dir, series) in zip(apps, charm_urls, args):
                ...
                resources = await self.model.add_local_resources(
                    ...
                    charm_url,
                    metadata,
                    resources=bundle.get("applications", {app_name: {}})[app_name].get(
                        "resources", {}
                    ),
                )
                ...
                if not self.model.connection().is_using_old_client:
                    origin.base = utils.get_local_charm_base(
                        series, charm_dir, client.Base
                    )
        ...
        for apps in bundle_apps + overlay_apps:
            for app_name, app in apps.items():
                if app and "options" in app:
                    if "config" in app["options"] and app["options"][
                        "config"
                    ].startswith("include-file"):
                        # resolve the file
                        if not bundle_dir:
                            raise NotImplementedError(
                                "unable to resolve paths for config:include-file for non-local charms"
                            )
                        try:
                            config_path = (
                                bundle_dir
                                / Path(app["options"]["config"].split("//")[1])
                            ).resolve()
                        except IndexError:
                            raise JujuError(
                                "the path for the included file should start with // and be relative to the bundle"
                            )
                        if not config_path.exists():
                            raise JujuError(
                                "unable to locate config file : %s for : %s"
                    ...
                    for option_key, option_val in app["options"].items():
                        if isinstance(option_val, str) and option_val.startswith(
                            "include-base64"
                        ):
                            # resolve the file
                            if not bundle_dir:
                                raise NotImplementedError(
                                    "unable to resolve paths for config:include-base64 for non-local charms"
                                )
                            try:
                                base64_path = (
                                    bundle_dir / Path(option_val.split("//")[1])
                                ).resolve()
                            except IndexError:
                                raise JujuError(
                                    "the path for the included base64 file should start with // and be relative to the bundle"
                                )

                            if not base64_path.exists():
                                raise JujuError(
                                    "unable to locate the base64 file : %s for : %s"
                                    % (base64_path, app_name)
                                )
    ...
    async def _download_bundle(self, charm_url, origin):
        ...
        resp = await self.charms_facade.GetDownloadInfos(
        ...
        with closing(bundle_resp), zipfile.ZipFile(
            io.BytesIO(bundle_resp.content)
        ) as archive:
    ...
    async def _resolve_charms(self):
        ...
        for name in self.applications:
            ...
            channel = (
                Channel.parse(spec["channel"])
                if "channel" in spec
                else Channel("latest", "stable")
            )
...
def is_local_charm(charm_url: str):
    return (
        charm_url.startswith(".")
        or charm_url.startswith("local:")
        or os.path.isabs(charm_url)
    )
...
class AddApplicationChange(ChangeInfo):
    ...
    async def run(self, context):
        ...
        resources = (
            context.bundle.get("applications", {})
    ...
    async def run(self, context):
        ...
        if not context.model.connection().is_using_old_client:
            params["base"] = client.Base(
                channel=utils.get_series_version(self.series), name="ubuntu"
            )
        else:
        ...
        if self.container_type == "lxc":
            log.warning(
                "Juju 2.0 does not support lxc containers. "
...
class AddUnitChange(ChangeInfo):
    _toPy = {"application": "application", "to": "to"}
...
class CreateOfferChange(ChangeInfo):
    ...
    async def run(self, context):
        ...
        for ep in self.endpoints:
            await context.model.create_offer(
                ep, offer_name=self.offer_name, application_name=application
            )
...
class ConsumeOfferChange(ChangeInfo):
    _toPy = {"url": "url", "application-name": "application_name"}
    ...
    async def run(self, context):
        ...
        local_name = await context.model.consume(
            self.url, application_alias=application
        )
        return local_name
...
class ScaleChange(ChangeInfo):
    _toPy = {"application": "application", "scale": "scale"}
    """
